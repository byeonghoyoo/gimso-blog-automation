...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("generateResponse").addEventListener("click", generateModelResponse);
    document.getElementById("saveEditedResponse").addEventListener("click", saveEditedResponse);
   // document.getElementById("downloadResponseDocx").addEventListener("click", handleDownloadResponseDocx);


    // ✅ 코드 추가: 네이버 블로그 새글 작성 창이 ID 포함해서 열리도록 수정
    document.getElementById("downloadResponseDocx").addEventListener("click", async () => {
        await handleDownloadResponseDocx(); // ✅ 기존 Word 변환 및 다운로드 실행

        // ✅ 저장된 네이버 ID 가져오기
        chrome.storage.local.get("savedNaverId", (data) => {
            const naverId = data.savedNaverId?.trim();

            if (naverId) {
                window.open(`https://blog.naver.com/PostWriteForm.naver?blogId=${naverId}`, "_blank");
            } else {
                alert("🚨 네이버 ID를 설정해주세요! (ID 입력 후 저장 버튼 클릭)");
            }
        });
    });
    ...
    // ✅ "HTML 코드 복사 하기" 클릭 시 티스토리 새글 작성 창 자동 실행 추가
    document.getElementById("copyHTML").addEventListener("click", async () => {
        // ✅ HTML 코드 클립보드에 복사
        let htmlText = document.getElementById("htmlOutput").value.trim();
        if (!htmlText) {
            alert("🚨 변환된 HTML 코드가 없습니다.");
            return;
        }
        ...
        try {
            ...
            // ✅ 저장된 티스토리 ID 가져오기
            chrome.storage.local.get("savedTistoryId", (data) => {
                const tistoryId = data.savedTistoryId?.trim();

                if (tistoryId) {
                    window.open(`https://${tistoryId}.tistory.com/manage/newpost/`, "_blank");
                } else {
                    alert("🚨 티스토리 ID를 설정해주세요! (ID 입력 후 저장 버튼 클릭)");
                }
            });
        } catch (err) {
            console.error("❌ 클립보드 복사 실패:", err);
            alert("🚨 HTML 코드 복사에 실패했습니다.");
        }
    });
    ...
    document.getElementById("copyHTML").style.display = "inline-block"; // 버튼 보이게 설정

    fetchBlogContent(); // ✅ 확장 프로그램이 실행되면 자동 실행
    document.getElementById("fetchBlogContent").addEventListener("click", fetchBlogContent);

    document.getElementById("generateResponse").addEventListener("click", () => {
        showModal(); // ✅ 클릭 시 모달 표시
        generateModelResponse();
    });


    // ✅ 페이지 로드 시 저장된 응답 & 프롬프트 불러오기
    loadSavedResponse();
    ...
    // ✅ 프롬프트 입력할 때마다 자동 저장
    document.getElementById("userPrompt").addEventListener("input", saveUserPrompt);
});
...
function loadSavedApiKey() {
    ...
    chrome.storage.local.get(`${model}ApiKey`, (data) => {
        ...
        if (apiKey) {
            ...
            console.log(`✅ 저장된 ${model} API 키 불러오기 완료:`, apiKey);
        } else {
            document.getElementById("apiKey").value = ""; // 키가 없으면 입력 필드 초기화
            console.warn(`⚠️ 저장된 ${model} API 키가 없습니다.`);
        }
    });
}
...
function saveApiKey() {
    ...
    const keyToSave = {};
    keyToSave[`${model}ApiKey`] = apiKey;

    chrome.storage.local.set(keyToSave, () => {
        if (chrome.runtime.lastError) {
            console.error("❌ API 키 저장 오류:", chrome.runtime.lastError);
            alert("🚨 API 키 저장 중 오류 발생!");
        } else {
            console.log(`✅ ${model} API 키 저장 완료:`, apiKey);
            alert(`✅ ${model} API 키가 저장되었습니다!`);
            loadSavedApiKey(); // 저장 후 바로 불러오기
        }
    });
}
...
function extractBlogDetails() {
    try {
        ...
        // ✅ 유튜브 댓글 크롤링_Ver1.7 추가함
        else if (document.querySelector("ytd-comment-thread-renderer")) {
            title = document.querySelector("meta[property='og:title']")?.content || document.title;

            let comments = [];
            document.querySelectorAll("ytd-comment-thread-renderer").forEach(comment => {
                let author = comment.querySelector("#author-text")?.innerText.trim() || "익명";
                let text = comment.querySelector("#content-text")?.innerText.trim() || "댓글 내용을 가져올 수 없습니다.";
                let time = comment.querySelector("#published-time-text")?.innerText.trim() || "시간 없음";
                let likes = comment.querySelector("#vote-count-middle")?.innerText.trim() || "0"; // 좋아요 수 추가

                comments.push(`👤 ${author} (${time}) 좋아요 ${likes}:\n${text}`);
            });
        ...
        // ✅ 네이버 카페 본문 크롤링_Ver1.7 추가함
        else if (document.querySelector("iframe#cafe_main")) {
            let cafeIframe = document.querySelector("iframe#cafe_main");
            let iframeDoc = cafeIframe.contentDocument || cafeIframe.contentWindow.document;
            title = iframeDoc.querySelector('meta[property="og:title"]')?.content || document.title;

            let contentElements = iframeDoc.querySelectorAll("div.se-main-container .se-text-paragraph");
            if (contentElements.length > 0) {
                content = Array.from(contentElements).map(el => el.innerText.trim()).join("\n\n");
            } else {
                content = "네이버 카페 본문을 찾을 수 없습니다.";
            }
        }
        ...
        console.error("❌ 블로그 정보 추출 오류:", error);
        return { title: "오류 발생", content: "오류가 발생했습니다." };
    }
}
...
document.addEventListener("DOMContentLoaded", async () => {
    const GOOGLE_SCRIPT_URL = "https://script.google.com/macros/s/AKfycby9lpCcFgM-xX7nZczEmUfP5b5ft74_kHMI_MMSX44Rp5146kFkDE8GXrAaRgN3EJ1d/exec"; // Google Apps Script URL
